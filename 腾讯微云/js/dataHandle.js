...
var dataAction = (function (){
	...
	return {
	    ...
	    hasChilds:function (datas,id){
	    	return !!dataAction.getChildrenById(datas,id).length;
	    },
	    ...
	    //作用:找到指定id的所有的子孙数据
	    getChildrenAll:function (datas,id){
            //通过循环数据，找到指定id的那条数据
            var arr = [];
            for( var i = 0; i < datas.length; i++ ){
                if( datas[i].id == id ){
                    arr.push(datas[i]);
                    var children = dataAction.getChildrenById(datas,datas[i].id);

                    for( var j = 0; j < children.length; j++ ){
                       arr = arr.concat(dataAction.getChildrenAll(datas,children[j].id));
                    }
                }
            }
            return arr;    
	    ...
	    getChildsAll:function (datas,id){
            ...
            for( var i = 0; i < datas.length; i++ ){
                if( datas[i].id == id ){
                    arr.push(datas[i]);
                    var childs = dataAction.getChildrenById(datas,datas[i].id);
